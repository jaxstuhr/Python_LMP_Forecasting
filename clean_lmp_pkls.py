...
def build_lmp(pkl_path): # function to build numpy array of [date-hour minute, lmp, node_id] for given node
    node_id = pkl_path.replace(".pkl", "") # extract node_id from path
    ...
    lmp_np_array = np.append(lmp_np_array, node_id_list, 1) # append node_ids to arrayy
    return lmp_np_array # return array
  
all_lmps_np_array = np.empty(shape = (0,3)) # initialize complete lmp dataset

for node_path in node_pkl_names: # iterate over nodes
   this_lmp_array = build_lmp(node_path) # build np array for each node
   all_lmps_np_array = np.append(all_lmps_np_array, this_lmp_array, 0) # append new node array to dataset
